    ...
    public static class AccionCuadradoMagico extends Accion {
        ...
        public boolean esAplicable(Estado es) {
            ...
            for (int i = 0 ; i < dimension ; i++) {
                ...
                if (fila == result)
                    return false;                                                                                        //Si la fila esta bien, aumenta la puntuacion
                if (columna == result)
                    return false;                                                                                               //Si la colummna esta bien, aumenta la puntuacion
                fila = 0;
                columna = 0;
            }
            if (diagonalizq == result)
                return false;                                                                                                   //Si la diagonal izquierda esta bien, aumenta la puntuacion
                                                                                                               //Si la diagonal izquierda esta bien, aumenta la puntuacion
            if (diagonalder == result)
                return false;                                                                                                   //Si la diagonal izquierda esta bien, aumenta la puntuacion

            return true;
            //Devuelve la meta*/
        }//si la fila en la que se va a insertar suma mas del resultado devolver falso
        ...
        public Estado aplicaA(Estado es) {
            ...
            estadoCuadradoMagico copia = new estadoCuadradoMagico(matriz);                                              //Creamos un nuevo estado que usa la matriz nueva como representacion

            for (int i = 0; i < dimension ; i++)
                for (int j = 0; j < dimension; j++) {
                    if (copia.matrizCuadradoMagico[i][j] == 0) {
                        copia.matrizCuadradoMagico[i][j] = acc;                                                         //Aplicamos la accion sobre el nuevo estado y luego lo devolvemos
                        return copia;
                    }
                }
            return copia;
